		   ...
		   $(this).addClass('active').siblings().removeClass('active');
			$.ajax({
				...
				success:function(data1){
					$projectGrid.masonry('destroy');
					$('#posts').append(data1);
					alert('1');
					//setTimeout(function(){
						$projectGrid.masonry( masonryOptions ); // re-initialize
						alert('3');
					//},2000);
...
$( ".cl_services" ).click(function() {
			...
			$.ajax({
				...
				if (jsonData.status == 'success') {
					...
					setTimeout(function(){
						$loader.removeClass('service_loading_loader').html('<a class="loadmore navigation" href="javascript:;">Load flere</a>')},2000);
				} else {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (jsonData.status == 'success') {
					...
					setTimeout(function(){
						$projectGrid.append( $appengrid ).masonry( 'appended', $appengrid, true );},2000);
