 ...
 if(count($posts_array) > 0){
	 foreach ( $posts_array as $post ) { 
		 ...
		 ?>
			...
			?>
            <tr><td><?php echo get_the_date('d | m | Y',$post->ID); ?></td><td><?php echo get_the_title($post->ID); ?></td><td><?php echo the_field('location',$post->ID); ?></td></tr>
         	<?php
			 }
		 $nopost++;
		} 
...
function auto_pr(){
	...
	?>
            <tr data-projid="<?php echo $post->ID;?>" class="projectrow">
                ...
                <td class="project_list_title"><?php echo get_the_title($post->ID);  ?></td>
