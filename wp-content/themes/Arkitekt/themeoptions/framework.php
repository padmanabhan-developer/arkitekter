        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    ...
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'backyard'), '<a href="'.esc_url('http://www.ydesignservices.com/') . '" target="_blank">' . __( 'Y Design Services', 'backyard' ) . '</a> v' . self::$_version ) . '</span>';
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    ...
                    $this->args['menu_title'] = __( 'Options', 'backyard' );
                ...
                if ( empty ( $this->args['page_title'] ) ) {
                    ...
                    $this->args['page_title'] = __( 'Options', 'backyard' );
...
// __construct()
            ...
            private function _internationalization() {
                ...
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'backyard' );
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'backyard' );

                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {

                        $event = __( 'clicking', 'backyard' );
                    ...
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'backyard' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    ...
                    $default_output = __( 'Default', 'backyard' ) . ": " . substr( $default_output, 0, - 2 );
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    ...
                    echo json_encode( array(
                        ...
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'backyard' ),
                ...
                if (!current_user_can ( $this->args['page_permissions'] )) {
                    ...
                    echo json_encode( array(
                        ...
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'backyard' ),
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'backyard' ) ) );
