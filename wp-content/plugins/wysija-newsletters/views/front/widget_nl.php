...
class WYSIJA_view_front_widget_nl extends WYSIJA_view_front {
	...
	function display($title='',$params,$echo=true,$iframe=false){
		...
		if(isset($params['form']) && (int)$params['form'] > 0) {
			...
			$submitbutton=$list_fields.'<input type="submit" '.$disabled_submit.' class="aa  wysija-submit wysija-submit-field" name="submit" value="'.esc_attr($submit_value).'"/>';
			...
			if($dataCf){
				...
				$data.='<p><input type="text" id="'.$form_id_real.'-wysija-to" class="1	'.$classValidate.'" value="'.$value_attribute.'" name="wysija[user][email]" />';
				...
				if(count($params['customfields'])>1){
					if(isset($params['labelswithin'])){
						 if($params['labelswithin']=='labels_within'){
							...
							$fieldstring='<label for="'.$fieldid.'">'.$field['label'].'</label><input type="text" id="'.$fieldid.'" class="2 '.$classValidate.'" name="wysija[user]['.$fieldKey.']" />';
						}
					}else{
						$fieldstring='<label for="'.$fieldid.'">'.$field['label'].'</label><input type="text" id="'.$fieldid.'" class="3 '.$classValidate.'" name="wysija[user]['.$fieldKey.']" />';
					}
				}else{
					if(isset($params['labelswithin'])){
						 if($params['labelswithin']=='labels_within'){
							...
							$fieldstring='<input type="text" id="'.$fieldid.'" class="4 '.$classValidate.'" name="wysija[user]['.$fieldKey.']" '.$value_attribute.'/>';
						}
					}else{
						$fieldstring='<input type="text" id="'.$fieldid.'" class=" 5'.$classValidate.'" name="wysija[user]['.$fieldKey.']" '.$value_attribute.'/>';
					}
				}
